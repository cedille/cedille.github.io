...
{-
-- μ and μ' can also be used for proofs. Due to limitations in type inference,
-- if you wish to prove some property `P n` by pattern-matching or recursion on
-- `n`, you must give the motive explicitly, via the @ syntax.
--
-- Below, `natCases` proves `P` holds for any `n` if it holds for `zero` and the
-- successor of any number. The motive `P` is given explicitly
-}

natCases : ∀ P: Nat ➔ ★. Π n: Nat. P zero ➔ (Π m: Nat. P (suc m)) ➔ P n
  = Λ P. λ n. λ z. λ s. μ' n @P {zero ➔ z | suc n ➔ s n}.

{-
-- `vecInd` below proves the induction principle for `Vec` using μ. It
-- introduces several new novel aspects of datatypes in Cedille
-}

vecInd : ∀ A: ★. ∀ P: (Π i: Nat. Vec ·A i ➔ ★). ∀ n: Nat. Π xs: Vec ·A n.
    P zero (vnil ·A) ➔ (∀ m: Nat. Π a: A. Π xs: Vec ·A m. P m xs ➔ P (suc m) (vcons -m a xs))
    ➔ P n xs
  = Λ A. Λ P. Λ n. λ xs. λ vn. λ vc. μ ih. xs @P {
  | vnil ➔ vn
...
eval : ∀ X: ★. Expr ·X ➔ X
  = Λ X. λ e. μ eval. e @(λ X: ★. λ x: Expr ·X. X) {
  | nExpr n ➔ n
  | bExpr b ➔ b
